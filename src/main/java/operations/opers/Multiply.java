...
public class Multiply implements IOperation {
    public void operate(CustomContainer context, String[] args) throws InvalidNumberOfArgs, StackDepthException {
        if (args.length != 1) {
            throw new InvalidNumberOfArgs("Passed " + (args.length-1) + " args in MULTIPLY");
        } else if (CustomContainer.stack_depth() < 2) {
            throw new StackDepthException("Less the 2 vars in stack during multiplying" + CustomContainer.stack_depth());
        } else {
